...
class FeedsHolder (v: View) : RecyclerView.ViewHolder(v) {
    fun bindItem(
        feed: Feed,
        checkedFeed: ArrayList<Feed>,
        onClick : ((feed: Feed) -> Unit),
        lastItem:Boolean
    ) {
        ...
        name.text = feed.title
        itemView.setOnClickListener { onClick.invoke(feed) }
    }
}
...
class SpeciesesHolder (v: View) : RecyclerView.ViewHolder(v) {
    fun bindItem(
        species: Species,
        checkedSpecies: ArrayList<Species>,
        onClick : ((species: Species) -> Unit),
        lastItem:Boolean
    ) {
        ...
        name.text = species.title
        itemView.setOnClickListener { onClick.invoke(species) }
    }
}